...
-   A [%g notebook "notebook" %] is a document that mixes text, code, and output in one file
    ...
    -   Marimo notebooks have the extension `.py` and look like Python scripts with special decorators
...
-   The dataset for this session is the Environment and Climate Change Canada Historical Climate Data [%b eccc-climate2025 %]
    -   Go to [ECCC Climate Data][eccc-climate] and search for a station near you (e.g., Toronto Pearson International Airport or Vancouver International Airport)
...
-   Paste this prompt into your LLM: "Using Polars, read a CSV file called climate.csv, skip the first row which is a title line, treat empty cells as missing, and print the first five rows."
...
-   Paste this prompt: "Using Polars, read climate.csv the same way, then compute and print the mean and median of the 'Mean Temp (°C)' column, excluding missing values."
-   The LLM will produce something like:

[%inc mean_median.py %]

-   Run the cell
    -   For most Canadian cities the annual mean will be between -10 and +15 degrees Celsius
...
-   The mean is the sum of all values divided by the count
    -   It is pulled toward extreme values: a few very cold winters raise the magnitude of negative values and pull the mean down
-   The median is the middle value when the data is sorted
    -   Exactly half the values are above it and half are below
    -   A few extreme months do not move it much
-   For Canadian climate data, the mean and median are often close because temperatures follow a roughly symmetric seasonal cycle
    -   In a dataset with [%g outlier "outliers" %]---hospital bills, income, city populations---they can differ dramatically
-   When outliers matter for your question, report the mean; when you want a typical value unaffected by extremes, report the median
...
-   Saving the notebook writes the current state---all cells, all outputs---to a `.ipynb` file on disk
...
The first likely cause is reading the wrong column---the code may be using a station ID or a precipitation total instead of mean temperature.
Check the column names printed in the output header to confirm which column was used.
The second likely cause is that `skip_rows=1` was wrong and the first data row was skipped or misaligned, shifting all values.
...
A few very cold winter months (perhaps during extreme cold snaps) pull the mean downward while the median stays closer to the centre of the distribution.
This is a common pattern for Canadian climate data, where extreme cold events are more severe than extreme warm events at most stations.
...
The visible result from yesterday does not mean the code will produce the same result today---it is a snapshot, not a live computation.
...
The median is the middle value and is not affected by those extremes, so it better represents what a typical property in the sample sells for.
