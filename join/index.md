...
-   Different teams collect different parts of the picture
    -   Temperature readings are collected continuously at each station; station locations and elevations are recorded once when the station is established
    -   Storing location data in every temperature row wastes space and creates inconsistency: if a station is relocated, you update one row in the station file instead of decades of readings
-   The dataset for this session is the Environment and Climate Change Canada Adjusted and Homogenized Canadian Climate Data (AHCCD) [%b ahccd2025 %]
    -   Download both files from [AHCCD][ahccd]: the annual temperature series (`ahccd_temp.csv`) and the station inventory (`ahccd_stations.csv`)
...
*What is a join, in plain English?*

-   A join combines two tables by matching rows that share a common value
    -   If both tables have a `station_id` column, a join finds every temperature row and attaches the matching station row to it
    -   The result is one wide row per temperature record, with both the temperature and the station's province in the same row
